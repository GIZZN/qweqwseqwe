...
interface PrivacySettings {
  ...
  always_on_top: boolean; // настройка "Поверх всех окон"
}
...
export default function Privacy({ onClose }: PrivacyProps) {
  ...
  const loadSettings = async () => {
    try {
      // Загружаем настройки приватности из логики 
      ...
      // проверка если стандартный курсор включен
      ...
      // проверка если режим "поверх всех окон" включен
      const isAlwaysOnTop = await invoke<boolean>('is_always_on_top_enabled');
      setAlwaysOnTopActive(isAlwaysOnTop);
      setSettings(prev => ({ ...prev, always_on_top: isAlwaysOnTop }));
      
      // проверка если защита от захвата экрана включена
      const isProtectionEnabled = await invoke<boolean>('is_protection_enabled');
      setProtectionActive(isProtectionEnabled);
      setSettings(prev => ({ ...prev, hide_from_screen_sharing: isProtectionEnabled }));
    } catch (error) {
  ...
  const checkAlwaysOnTopStatus = async () => {
    try {
      ...
      console.error('Error checking always on top status:', error);
    }
  };
  ...
  const handleToggle = async (key: keyof PrivacySettings, value: boolean) => {
    ...
    if (key === 'hide_from_taskbar') {
      ...
      try {
        // Активируем/деактивируем в Rust (система курсоров на уровне ОС)
        await invoke('set_standard_cursor', { enabled: value });
        setStandardCursorActive(value);
        
        // Компонент StandardCursor автоматически подхватит изменения через polling
        // поэтому дополнительных действий не требуется
        
        console.log(`Стандартный курсор ${value ? 'включен' : 'отключен'} (обе системы)`);
      } catch (error) {
      ...
      try {
        ...
        setAlwaysOnTopActive(value);
        console.log(`Режим "Поверх всех окон" ${value ? 'включен' : 'отключен'}`);
      } catch (error) {
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {alwaysOnTopActive && (
                    <span className="hidden xs:flex px-2 py-1 bg-purple-600/20 border border-purple-600/30 text-purple-300 text-xs rounded-full items-center flex-shrink-0">
              ...
              <div className="xs:hidden flex items-center space-x-2">
                ...
                {standardCursorActive && (
                  <div className="w-2 h-2 bg-yellow-200 rounded-full animate-pulse flex-shrink-0"></div>
                )}
                {alwaysOnTopActive && (
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="flex flex-col xs:flex-row xs:items-start xs:justify-between gap-3 xs:gap-0">
                    <div className="flex-1 xs:pr-4">
                      <h3 className="font-medium text-white mb-1 text-sm sm:text-base">Поверх всех окон</h3>
                      <p className="text-xs sm:text-sm text-white/60">
                        Окно приложения будет отображаться поверх всех других окон.
                        <br />
                        <span className="text-white/40 text-xs">
                          Полезно для постоянного доступа к приложению во время работы с другими программами
                        </span>
                      </p>
                    </div>
                    <button
                      onClick={() => handleToggle('always_on_top', !settings.always_on_top)}
                      className={`ui-switch relative inline-flex h-5 w-9 sm:h-6 sm:w-11 items-center rounded-full transition-all duration-300 ease-in-out flex-shrink-0 ${
                        settings.always_on_top ? 'bg-[#1CCDAA]' : 'bg-white/20'
                      }`}
                    >
                      <span
                        className={`ui-switch__thumb inline-block h-3 w-3 sm:h-4 sm:w-4 transform rounded-full bg-white transition-all duration-300 ease-in-out ${
                          settings.always_on_top ? 'translate-x-5 sm:translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col xs:flex-row xs:items-start xs:justify-between gap-3 xs:gap-0">
                    <div className="flex-1 xs:pr-4">
                      ...
                      <p className="text-xs sm:text-sm text-white/60">
                        Курсор всегда будет отображаться как стандартная стрелка во всём приложении.
                        <br />
                        <span className="text-white/40 text-xs">
                          Управляет курсором как в интерфейсе приложения, так и при взаимодействии с окном (ресайз, перемещение)
                        </span>
                      </p>
                    </div>
                    <button
